class FriendshipsController < ApplicationController
  ...
  def index
    ...
    @acceptedFriendships = Friendship.accepted.getMutualFriendships(@user.id)

    @friends = getFriends(@acceptedFriendships, @user.id)

    #@accepted_friended_friendships = Friendship.accepted.get_friended_friendships(@user.id)
    #@accepted_friender_friendships = Friendship.accepted.get_friender_friendships(@user.id)
    @outgoingFriendships = Friendship.pending.getOutgoingFriendships(@user.id)
    @incomingFriendships = Friendship.pending.getIncomingFriendships(@user.id)
  end
    ...
    respond_to do |format|
      if @friendship.update_attributes(friendship_params)
        if @friendship.friender.notification_configuration.notify_friendships == true
          NotificationWorker.perform_async(@friendship.id, "#{@friendship.class.name.demodulize}", 'Accept')
        end
  ...
  def destroy
    @friendship = Friendship.find(params[:id])
    @friended_user = @friendship.getFriend(current_user.id)
